...
def log(value, status='Seting Proxy & Port', color='[CC]'):
    value = colors('{color}''[P1]''[{color}] [G1]{color}{status} [Y1]{color}{value}[CC]'.format(
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('[CC]____________[G1]kurang [Y1]tamvan [G1]boss[CC]________________[Y1]_________________________________________________[P1][[R1]]'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('[CC]______________________________________________[G1]___________________[G1]P[R1]a[Y1]g[P1]a[CC]r[G1] I[P1]n[G1]t[R2]e[Y1]r[CC]n[G1]e[R2]t[G1]________________[P1][[R1]]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
    	 	  '[P1]==================================================[G1]',
       '[R1]      Telkomsel IlmuPedia Versi Terakhir','[R1]',
       '[P1]==================================================','[G1]',
       '[CC]_________________ [R1]Rendy [CC]0zunu ____________________','[CC]',
       '[CC]==================================================','[Y1]',
       '[R1]__________[R1]C[P1]h[Y1]a[G1]n[CC]n[R2]e[P1]l [Y1]Y[G1]o[CC]u[R1]t[Y1]u[G1]b[P1]e [G1]P[R1]a[Y1]g[P1]a[CC]r[G1] I[P1]n[G1]t[R2]e[Y1]r[CC]n[G1]e[R2]t __________','[CC]',
       '[Y1]      Subcribe untuk update script tiap hari','[CC]',
       '[R1]   join group[Y1] :Pagar Internet','[Y1]',
       '[R1]   Tester     [Y1]:NTB,Jateng,Jatim','[Y1]',
       '[R1]___________________________________________________','[R1]',
       '[R1]   Sunek      [Y1]: On/0f Data Mode Pesawat','[Y1]'
       '[G1]===================================================','[G1]',
    ])))    
    inject('127.0.0.1', '9090').start()

if __name__ == '__main__':
    main()
#Creator By Gretongers Indonesia
